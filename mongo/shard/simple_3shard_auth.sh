...
key="$HOME/mongodb/keys/MyKeyFile"
dirs="
$HOME/data/db/1
$HOME/data/db/2
$HOME/data/db/3
...
do
        ...
        if [ -d "$dir" ]
            then
            $mongod --shardsvr --dbpath $dir --port 1000$port --fork --logpath $ldir/shard.$port.log --keyFile $key # Start MongoDB Sharded Servers
            continue # Go to next iteration of $dir
        else
            mkdir -p $dir
            $mongod --shardsvr --dbpath $dir --port 1000$port --fork --logpath $ldir/shard.$port.log --keyFile $key # Start MongoDB Sharded Servers, if directories don't exist
        fi
done
...
if [ -d "$cdir" ]
        ...
        sleep 5
        $mongos --configdb localhost:$s_port --chunkSize 1 --fork --logpath $ldir/mongos.log --keyFile $key
else
        mkdir -p $cdir
        $mongod --configsvr --dbpath $cdir --port $s_port --fork --logpath $ldir/configdb.log
        sleep 5
